...
struct OllamaClient {
    ...
    /// Every preset CONTAINS the full faithful core verbatim and
    /// only APPENDS a style layer, so the reformat-don't-answer / don't-invent
    /// guard is identical in all of them. `.faithful` is byte-identical to the
